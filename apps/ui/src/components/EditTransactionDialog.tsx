...
export function EditTransactionDialog({ open, onClose, portfolioId, transaction }: EditTransactionDialogProps) {
  ...
  useEffect(() => {
    if (transaction && open) {
      setFormData({
        ...
        priceEur: transaction.priceEur.toFixed(2),
        feeEur: transaction.feeEur?.toFixed(2) || '0',
        executedAt: new Date(transaction.executedAt).toISOString().slice(0, 16),
        note: transaction.note || '',
      });
      setSelectedInstrument(transaction.instrument);
    }
  }, [transaction, open]);
  ...
  const updateMutation = useMutation({
    mutationFn: (data: any) => api.transactions.update(portfolioId, transaction.id, data),
    onSuccess: () => {
    ...
    onError: (err: any) => {
      setError(err.message || 'Errore durante la modifica della transazione');
    },
  });
  ...
  return (
    <Dialog open={open} onClose={onClose} title="Modifica Transazione" maxWidth="lg">
