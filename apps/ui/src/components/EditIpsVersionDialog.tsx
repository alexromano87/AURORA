...
interface EditIpsVersionDialogProps {
  open: boolean;
  onClose: () => void;
  version: any;
}

export function EditIpsVersionDialog({ open, onClose, version }: EditIpsVersionDialogProps) {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: any) => api.ips.updateVersion(version.id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['ips'] });
      onClose();
    },
    onError: (err: any) => {
      setError(err.message || 'Errore durante la modifica della versione');
    },
  });
  ...
  return (
    <Dialog open={open} onClose={onClose} title={`Modifica Versione ${version?.version}`}>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              disabled={updateMutation.isPending}
            />
            <label htmlFor="esg-edit" className="text-sm font-medium">
              Investimenti ESG
            </label>
          </div>
