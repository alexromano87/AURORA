...
interface CreateIpsDialogProps {
  open: boolean;
  onClose: () => void;
}

export function CreateIpsDialog({ open, onClose }: CreateIpsDialogProps) {
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => api.ips.createPolicy(data),
    onSuccess: () => {
    ...
    onError: (err: any) => {
      setError(err.message || 'Errore durante la creazione della policy IPS');
    },
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.timeHorizon || !formData.age || !formData.goal) {
  ...
  return (
    <Dialog open={open} onClose={onClose} title="Crea Nuova Policy IPS">
      <form onSubmit={handleSubmit} className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              disabled={createMutation.isPending}
            />
            <label htmlFor="esg" className="text-sm font-medium">
              Investimenti ESG
            </label>
          </div>
        ...
        <div className="flex justify-end gap-3 pt-4 border-t">
          ...
          >
            {createMutation.isPending ? 'Creazione...' : 'Crea Policy'}
          </button>
        </div>
      </form>
    </Dialog>
